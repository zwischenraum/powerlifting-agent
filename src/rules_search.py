...
class RulesSearch:
    ...
    def _init_collection(self):
        ...
        try:
            ...
            collection_info = self.qdrant.get_collection(collection_name)
            logging.info(
                f"Created new collection with {collection_info.points_count} vectors"
            )
    ...
    def _upload_texts(self):
        ...
        if collection_info.points_count == 0:
            logging.debug(
                f"Getting embeddings for {len(self.rules_chunks)} text chunks..."
            )
            ...
            try:
                self.qdrant.upload_points(
                    collection_name="rules", points=points, wait=True
                )
            ...
            logging.info(
                f"Collection already contains {collection_info.points_count} vectors"
            )
    ...
    def search(self, query: str, k: int = 3) -> List[Dict]:
        ...
        bm25_rank = np.empty_like(bm25_ranks)
        for rank, chunk_id in enumerate(bm25_ranks):
        ...
        for idx in range(len(self.rules_chunks)):
            rrf_scores[idx] = (1 / (self.RRF_C + bm25_rank[idx])) + (
        ...
        for idx in top_k_idx:
            ...
            semantic_score = semantic_results[
                semantic_ranks[idx]
            ].score  # Extract the score from ScoredPoint
